...
import common.ErrorHelper.ErrorHelper
...
object HttpHelper {
    ...
    def postData(data: String, dataserver: String, headerInfo: Map[String, String]): Boolean = {
        try {
            ...
            case ex: Exception =>
                LogHelper.errLoger(s"connect to ${dataserver} have exception. ${ex.getMessage()}");
                return false;
        }
    }
    ...
    def httpGetRequest(urlStr: String, headerInfo: Map[String, String]): Map[String, String] =  {
        ...
        try {
            ...
            case ex: Exception =>
                LogHelper.errLoger(s"connect to ${urlStr} have exception. ${ex.getMessage()}");
        }
    ...
    def getWebPage(host: String, path: String): String =  {
        try {
            ...
            case ex: Exception =>
                LogHelper.errLoger(s"connect to ${host}${path} have exception. ${ex.getMessage()}");
    ...
    def getWebPage(url: String): String = {
        try {
            ...
            case ex: Exception =>
            LogHelper.errLoger(s"connect to ${url} have exception. ${ex.getMessage()}");
            return Unit.toString;
        }
    }
}
