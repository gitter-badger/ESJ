...
// import config.ConfigurationEngine
...
object HbaseHelper {
    ...
    def getHbaseConnection(): Connection = {
        ...
        return ConnectionFactory.createConnection(getHbaseConfig(conf_kv));
    }
    ...
    def createTable(tableName: String, familys: ArrayBuffer[String], kv: Map[String, String]): Boolean = {
        try {
            ...
            if (admin.tableExists(table)) {
                LogHelper.warningLoger(s"the table: ${tableName} was exist\n");
                return true;
            }
            ...
            case ex: Exception =>
                LogHelper.errLoger(ex.getMessage() + "\n");
                return false;
        }
    }
    ...
    def createTableForce(tableName: String, familys: ArrayBuffer[String], kv: Map[String, String]): Boolean =  {
        try {
            ...
            return createTable(tableName, familys, kv);
        } catch {
            case ex: Exception =>
                LogHelper.errLoger(ex.getMessage() + "\n");
                return false;
    ...
    def addColumn(tableNameStr: String, key: String, family: String, columnName: String, columnValue: String): Boolean = {
        try {
            ...
            if (!admin.tableExists(tableName)) {
                LogHelper.errLoger(s"the table: ${tableNameStr} is not exist\n");
                return false;
            }else {
    ...
    def getColumn(tableNameStr: String, key: String, family: String, column: String): String = {
        try {
            ...
            if (!admin.tableExists(tableName)) {
                ...
                if (value == null) return Unit.toString;
                return value;
            }
        } catch {
            case ex: Exception =>
    ...
    def getColumnAtAllVersion(tableNameStr: String, key: String, family: String, column: String): ArrayBuffer[String] = {
        ...
        try {
            ...
            case ex: Exception =>
                LogHelper.errLoger("getColumnAtAllVersion: " + ex.getMessage() + "\n");
        }
    ...
    def getRow(tableNameStr: String, rowKey: String, family: String, column: String): Map[String, String] = {
        ...
        try {
            val tableName = TableName.valueOf(tableNameStr)
            if (!admin.tableExists(tableName)) {
                LogHelper.errLoger(s"HbaseHelper: getRow: ${tableName} not exist")
            } else {
                val table = conn.getTable(tableName)
            ...
            case ex: Exception =>
            LogHelper.errLoger(s"HbaseHelper: getRow: ${ex.getMessage()}\n")
        }
